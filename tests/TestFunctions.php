...
class TestFunctions extends TestCase {
	...
	public function testApPostEditLink() {
		...
		if ( ! \is_multisite() ) {
			...
			$this->assertEquals(
				home_url("/") . "?page_id=". ap_opt('ask_page') ."&id=" . $id->q . "&__nonce=" .$nonce,
		...
		if ( ! \is_multisite() ) {
			...
			$this->assertEquals(
				home_url("/") . "?page_id=". ap_opt('base_page') ."&id=" . $id->a . "&__nonce=" .$nonce,
	...
	public function testAPUserDisplayNameForPassingUserReturnHTMLArg() {
		...
		$user = get_user_by( 'id', $user_id );
		$this->assertEquals(
			'<a href="' . esc_url( ap_user_link( $user_id ) ) . '" itemprop="url"><span itemprop="name">Test User</span></a>',
			ap_user_display_name( [ 'user_id' => $user->ID, 'html' => true ] )
		);
	}
