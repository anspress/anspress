...
class AnsPress_Admin {
	...
	public static function init() {
		...
		add_action( 'save_post', array( __CLASS__, 'ans_parent_post' ), 10, 2 );
		...
		add_action( 'parent_file', array( __CLASS__, 'fix_active_admin_menu' ), 1000 );
		add_action( 'admin_init', array( __CLASS__, 'init_actions' ) );
		add_action( 'parent_file', array( __CLASS__, 'tax_menu_correction' ) );
		add_action( 'load-post.php', array( __CLASS__, 'question_meta_box_class' ) );
		add_action( 'load-post-new.php', array( __CLASS__, 'question_meta_box_class' ) );
		add_action( 'admin_menu', array( __CLASS__, 'change_post_menu_label' ) );
		add_filter( 'wp_insert_post_data', array( __CLASS__, 'post_data_check' ), 99 );
		add_action( 'admin_head-nav-menus.php', array( __CLASS__, 'ap_menu_metaboxes' ) );
		add_filter( 'posts_clauses', array( __CLASS__, 'join_by_author_name' ), 10, 2 );
		add_action( 'get_pages', array( __CLASS__, 'get_pages' ), 10, 2 );
		add_action( 'wp_insert_post_data', array( __CLASS__, 'modify_answer_title' ), 10, 2 );
		add_action( 'admin_footer-edit.php', array( __CLASS__, 'append_post_status_list_edit' ) );
		add_action( 'admin_footer-post.php', array( __CLASS__, 'append_post_status_list' ) );
		add_action( 'admin_footer-post-new.php', array( __CLASS__, 'append_post_status_list' ) );
		add_action( 'admin_post_anspress_update_db', array( __CLASS__, 'update_db' ) );
		add_action( 'admin_post_anspress_create_base_page', array( __CLASS__, 'anspress_create_base_page' ) );
